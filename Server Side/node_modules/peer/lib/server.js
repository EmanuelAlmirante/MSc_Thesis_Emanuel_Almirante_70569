...
app._checkKey = function(key, ip, cb) {
    if (key == this._options.key) {
        if (!this._clients[key]) {
            this._clients[key] = {};
            this._clientToContent[key] = {};
            this._contentToClient[key] = {};
        }
...
app._initializeHTTP = function() {
    ...
    // Post a peer as a streamer.
    this.get('/:key/streamer/:id/:streamhash', function(req, res, next) {

        var key = req.params.key;
        var id = req.params.id;
        var chash = req.params.streamhash;

        if (self._clients[key]) {

            if (self._clients[key][id]) {

                if (self._clientToContent[key][id] === undefined) self._clientToContent[key][id] = new Array();
                if (self._clientToContent[key][id].indexOf(chash) == -1) self._clientToContent[key][id].push(chash);


                if (self._contentToClient[key][chash] === undefined) self._contentToClient[key][chash] = new Array();
                if (self._contentToClient[key][chash].indexOf(id) == -1) self._contentToClient[key][chash].push(id);
    ...
    this.get('/:key/deletestreamer/:id/:streamhash', function(req, res, next) {
        ...
        if (self._clients[key]) {
            ...
            self._checkAllowsDiscovery(key, function(isAllowed) {
                ...
                if (isAllowed && self._contentToClient[key][chash] !== undefined) {

                    var index = self._contentToClient[key][chash].indexOf(id);

                    if (index > -1) {

                        self._contentToClient[key][chash].splice(index, 1);
    ...
    //Get a list of all streamers.
    this.get('/:key/streams/:streamhash', function(req, res, next) {
        ...
        if (self._clients[key]) {
            ...
            self._checkAllowsDiscovery(key, function(isAllowed) {
                ...
                if (isAllowed && self._contentToClient[key][chash] !== undefined) {
                    ...
                    res.send(self._contentToClient[key][chash]);
    ...
    // Post a peer as a secondary streamer.
    this.get('/:key/secondarystreamer/:id/:secondarystreamhash', function(req, res, next) {

        var key = req.params.key;
        var id = req.params.id;
        var chash = req.params.secondarystreamhash;
        ...
        if (self._clients[key]) {
            ...
            if (self._clients[key][id]) {
                ...
                console.log(self._contentToClient);
    ...
    //Deletes a streamer from the secondary list.
    this.get('/:key/deletesecondarystreamer/:id/:secondarystreamhash', function(req, res, next) {
    ...
    //Get a list of all secondary streamers.
    this.get('/:key/secondarystreams/:secondarystreamhash', function(req, res, next) {
    ...
    // JD
    // Get a list of all peers for a key with that content, enabled by the `allowDiscovery` flag.
    this.get('/:key/peers/:contenthash', function(req, res, next) {
        var key = req.params.key;
        var chash = req.params.contenthash;
        if (self._clients[key]) {
            self._checkAllowsDiscovery(key, function(isAllowed) {
                if (isAllowed && self._contentToClient[key][chash] !== undefined) {
                    res.send(self._contentToClient[key][chash]);
    ...
    // JD
    // Post a peer as a source for that content.
    // TODO - make it a POST
    this.get('/:key/content/:id/:contenthash', function(req, res, next) {
        var key = req.params.key;
        var id = req.params.id;
        var chash = req.params.contenthash;
        if (self._clients[key]) {
...
app._removePeer = function(key, id) {
    if (this._clients[key] && this._clients[key][id]) {
        ...
        /* JD Mod - remove cross mapping of content */
        for (var i in this._clientToContent[key][id]) {
            var obj = this._clientToContent[key][id][i];
            this._contentToClient[key][obj].splice(this._contentToClient[key][obj].indexOf(id), 1);
        }
        delete this._clientToContent[key][id];
        /* End JD mod */
