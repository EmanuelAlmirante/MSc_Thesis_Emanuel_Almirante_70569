(function e(t, n, r) {
    ...
    2: [function(require, module, exports) {
        ...
        //Send content hash from a content. EMANUEL.
        DataConnection.prototype.getContent = function(contenthash) {
            if (!this.open) {
            ...
            if (this._reliable) {
                ...
                // serialization.
                this._reliable.send(data, false, 'content');
                return;
            }
            console.log('trying to emit... ' + contenthash);
            this._reliable.send(data);
            this.emit('content', contenthash);
        }
    ...
    }, { "./adapter": 1, "./dataconnection": 2, "./mediaconnection": 4, "./negotiator": 5, "./peer": 6, "./socket": 7, "./util": 8, "js-binarypack": 10 }],
            ...
            pc.oniceconnectionstatechange = function() {
                switch (pc.iceConnectionState) {
                    ...
                    case 'failed':
                        ...
                        connection.close();
                        break;
                        //EMANUEL: TALVEZ TENHA DE MUDAR ISTO!!!!!!!!!!!!!!!!!!!!!
                        //case 'disconnected':
                        //util.log('iceConnectionState is disconnected, closing connections to ' + peerId);
                        //connection.close();
                        //break;
    ...
    6: [function(require, module, exports) {
        ...
        /* Announce peer as source of a certain content. Adapted from JD. EMANUEL.*/
        Peer.prototype.announceContent = function(hash, cb) {
            cb = cb || function() {};
            var self = this;
            var http = new XMLHttpRequest();
            var protocol = this.options.secure ? 'https://' : 'http://';
            var url = protocol + this.options.host + ':' + this.options.port +
                this.options.path + this.options.key + '/content/' + this.id + '/' + hash;
            var queryString = '?ts=' + new Date().getTime() + '' + Math.random();
            url += queryString;
        ...
        /* Get a list of peers that have a certain content. Adapted from JD. EMANUEL.*/
        Peer.prototype.listAllPeersWithContent = function(hash, cb) {
            ...
            var protocol = this.options.secure ? 'https://' : 'http://';
            var url = protocol + this.options.host + ':' + this.options.port +
                this.options.path + this.options.key + '/peers/' + hash;
            var queryString = '?ts=' + new Date().getTime() + '' + Math.random();
            url += queryString;
